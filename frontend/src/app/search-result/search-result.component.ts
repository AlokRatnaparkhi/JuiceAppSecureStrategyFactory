...
declare const SecureContext:any;
...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit () {
    ...
    forkJoin([quantities, products]).subscribe(([quantities, products]) => {
      ...
      for (let i = 1; i <= Math.ceil(this.dataSource.data.length / 12); i++) { console.log('D3')
  ...
  filterTable () {
   ...
   let fc=new Factory();
    let context=new SecureContext(fc.getSecureStrategy("XSS"));
    if(context.executeSecureStrategy(queryParam))
